...
#   valid face output.
def run_images(valid_output, validated_image_filename, facenet_exec_net, input_image_filename_list, input_blob,
               output_blob):
    ...
    for input_image_file in input_image_filename_list:
        ...
        if (face_match(valid_output, test_output)):
            ...
            text_color = (0, 0, 255)
            print(
                RED + ' FAIL!  File ' + input_image_file + ' does not match ' + validated_image_filename + "\n" + NOCOLOR)

        overlay_on_image(infer_image, input_image_file, matching)

        cv2.putText(infer_image, match_text + " - Hit key for next.", (30, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.5,
                    text_color, 1)
